...
object Layout {
    ...
    fun checkGridSize(tw: Int, th: Int) {
        if (StackManager.stackGrid.size < th || (StackManager.stackGrid.isNotEmpty() && StackManager.stackGrid[0].size < tw)) {
            StackManager.stackGrid = Array(th) { arrayOfNulls(tw) }
            isClean = false
        }
    }
